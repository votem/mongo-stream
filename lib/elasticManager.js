...
// insert event format https://docs.mongodb.com/manual/reference/change-events/#insert-event
  insertDoc(changeStreamObj) {
    ...
    const mapObj = this.mappings[changeStreamObj.ns.coll];

    // defaults to the parentId loaded into the config, but tries to guess what the field should be passed on the parent name
    const parentIdFieldName = mapObj.parentId || (mapObj._parent && mapObj._parent.type && `${mapObj._parent.type}Id`);
    ...
    this.bulkOp.push({
        index:  {
          ...
          _type: mapObj.type,
          _id: esId,
          _parent: esReadyDoc[parentIdFieldName],
          _versionType: mapObj.versionType,
          _version: versioning.getVersionAsInteger(esReadyDoc[mapObj.versionField])
        }
      });
  ...
  async deleteDoc(changeStreamObj) {
    ...
    const { parentId, version } = await this.getExistingDoc(mapObj, esId).catch((err) => {
