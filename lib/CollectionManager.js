...
class CollectionManager {
  ...
  async dumpCollection() {
    // const todoQuery = this.dumpProgress.count
      ...
      // : {};
    // let cursor = this.collection.find(todoQuery).batchSize(BATCH_SIZE);
    ...
    while (await cursor.hasNext()) {
      ...
      if (bulkOp.length !== 0 && bulkOp.length % (CollectionManager.elasticManager.bulkSize * 2) === 0) {
        ...
        logger.info(`${this.collectionName} progress: ${this.dumpProgress.count}/${totalCount} -- ${rate)} docs/sec -- eta ${eta}s`);
